...
export default class TagsService {
  static async buildTagIssues(projectKey, tag) {
    ...
    if (issues && rules) {
      ...
      const optimizedRules = rules.filter((val) => !affectedRules.includes(val.key))
      ...
      if (affectedRules && optimizedRules) {
        ...
        if (tagIssue.optimizedRules.value) {
          tagIssue.optimizedRules.percentage = (optimizedRules.length / rules.length) * 100
        }
        ...
        const secondSlice = firstSlice + (parseFloat(tagIssue.minorIssues.percentage) / 100) * 360
        const thirdSlice =
          parseFloat(secondSlice) + (parseFloat(tagIssue.majorIssues.percentage) / 100) * 360
        const lastSlice =
          parseFloat(thirdSlice) + (parseFloat(tagIssue.criticalIssues.percentage) / 100) * 360
        ...
        const cercleStyle =
          `${
            'white conic-gradient(' + 'from 0deg at 50% 50%,' + '#85BB2F 0deg, #85BB2F '
          }${firstSlice}deg,` +
