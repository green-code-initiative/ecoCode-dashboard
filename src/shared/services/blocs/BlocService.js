import { findTreeStoreKey, totalAppCodeLinesStoreKey } from '../../common/storageKeys'
import Bloc from '../../models/Bloc'
import { getStoredData } from '../../storage/DataStorage'

export default class BlocService {
  // Generates the Bloc entity from the stored data needed
  async build() {
  ...
  static async buildBloc(child, totalLines) {
    ...
    if (bloc.majorIssues && bloc.minorIssues && bloc.criticalIssues) {
      bloc.numberOfErrors =
        bloc.minorIssues.length + bloc.majorIssues.length + bloc.criticalIssues.length
    }
