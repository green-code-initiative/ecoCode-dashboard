...
export default class SideBarService {
  ...
  static async calculateOptimizedRules() {
    ...
    const issuesOfEachRule = await getStoredData(numberIssuesOfEachRuleStoreKey)
    ...
    if (issuesOfEachRule && totalRules) {
      ...
      res.optimizableRulesPercentage = (optimizedRules * 100) / totalRules.length

      if (res.optimizableRulesPercentage === 100) {
        res.optimizableRulesCircleColorClass = 'circle-shadow-inset--green'
      } else if (res.optimizableRulesPercentage >= 66 && res.optimizableRulesPercentage <= 99) {
        res.optimizableRulesCircleColorClass = 'circle-shadow-inset--yellow'
      } else if (res.optimizableRulesPercentage >= 33 && res.optimizableRulesPercentage <= 65) {
        res.optimizableRulesCircleColorClass = 'circle-shadow-inset--orange'
      } else if (res.optimizableRulesPercentage >= 0 && res.optimizableRulesPercentage <= 33) {
