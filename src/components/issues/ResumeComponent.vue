<template>
  <div class="resume-component">
    ...
    <div
      :class="['circle-shadow-inset', state.optimizableRules.optimizableRulesCircleColorClass]"
      :style="{
        '--angle': `${(state.optimizableRules.optimizableRulesPercentage / 100) * 360}deg`
      }"
    >
      <p>
...
import { rulesStoreKey, totalAppCodeLinesStoreKey } from '../../shared/common/storageKeys'
...
const linesOfCodeFormatted = computed(() =>
  Math.abs(state.linesOfCode) > 999
    ? `${Math.sign(state.linesOfCode) * (Math.abs(state.linesOfCode) / 1000).toFixed(1)}k`
