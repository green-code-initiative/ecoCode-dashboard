<template>
  ...
  >
    ...
    <strong v-if="bloc.blocType !== 'optimized' && bloc.blocType !== 'not-covered'">
...
const indirectErrors = computed(() => props.bloc.numberOfErrors - props.bloc.numberOfDirectErrors)
const directErrors = computed(() => props.bloc.numberOfDirectErrors)
const directText = computed(() => (directErrors.value > 1 ? 'Directs' : 'Direct'))
const indirectText = computed(() => (indirectErrors.value > 1 ? 'Indirects' : 'Indirect'))
</script>
